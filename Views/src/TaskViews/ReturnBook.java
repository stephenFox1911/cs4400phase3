...
public class ReturnBook extends JPanel {
	...
	public ReturnBook(UserView in) {
		...
		JLabel lblReturnedInDammeged = new JLabel(
				"Returned in Dammeged Condition");
		...
		// TODO: Choice is out of scope for action listener. Needs to be
		// resolved
		choice = new Choice();
		choice.add("No");
		choice.add("Yes");
		...
		btnCheckId.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String query = String
						.format("SELECT co_username,co_book_isbn,co_bcopy_no,DATEDIFF(NOW(),est_return_date) AS days_overdue FROM ISSUE WHERE issue_id=\'%s\'",
								textIssueID.getText());
				DBdriver db = new DBdriver();
				ResultSet result = db.sendQuery(query);
				try {
					if (result.next()){
					txtUserName.setText(result.getString("co_username"));
					textISBN.setText(result.getString("co_book_isbn"));
					textField.setText(result.getString("co_bcopy_no"));
					daysOverdue = result.getInt("days_overdue");
					} else {
						JOptionPane.showMessageDialog(null,"Did not find hold.");
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	public void returnBook(String issueID) {
		...
		try {
			...
			if (daysOverdue > 0) {
				String getBookCostQuery = String.format(
						"SELECT cost FROM BOOK WHERE isbn=\"%s\"",
						textISBN.getText());
				ResultSet bookCostResult = db.sendQuery(getBookCostQuery);
				bookCostResult.next();
				float bookCost = bookCostResult.getInt("cost");
				String updatePenaltiesQuery = String
						.format("UPDATE NON_STAFF_USER SET total_penalties = total_penalties+%f WHERE username=\"%s\"",
			...
			String unflagCopyQuery = String
					.format("UPDATE COPY SET is_checked_out = FALSE WHERE copy_number=\"%s\" AND book_isbn=\"%s\";",
							textField.getText(), textISBN.getText());
			db.sendUpdate(unflagCopyQuery);

			// Check for a future hold request on the book. If one exists, place
			// the book on hold.
			String checkFutureHoldQuery = String
					.format("SELECT future_requester FROM COPY WHERE copy_number=\"%s\" AND book_isbn=\"%s\" AND future_requester IS NOT NULL",
							textField.getText(), textISBN.getText());
			ResultSet futureHoldResult = db.sendQuery(checkFutureHoldQuery);

			if (futureHoldResult.next()) {
				String checkForOtherHoldsQuery = String
						.format("SELECT COUNT(*)>0 FROM NON_STAFF_USER, ISSUE WHERE NON_STAFF_USER.username=ISSUE.co_username AND ISSUE.co_book_isbn=\"%s\"",
								textISBN.getText());
				ResultSet checkForOtherHoldsResult = db
						.sendQuery(checkForOtherHoldsQuery);

				// Create the hold, set future requester to null
				if (!checkForOtherHoldsResult.next()) {
					String createNewHoldQuery = String
							.format("INSERT INTO ISSUE (est_return_date, date_created, co_username, co_bcopy_no, co_book_isbn, extensioncount) VALUES (DATE_ADD(CURDATE(), INTERVAL 17 DAY), CURDATE(),\"%s\",\"%s\",\"%s\", 0)",
									txtUserName.getText(), textField.getText(),
									textISBN.getText());
					db.sendUpdate(createNewHoldQuery);
					String clearFutureRequesterQuery = String
							.format("UPDATE COPY SET future_requester = NULL WHERE copy_number=\"%s\" AND book_isbn=\"%s\"",
									textField.getText(), textISBN.getText());
